...
namespace Napoleon.Db
{
    public static class DbHelper
    {
        ...
        /// Author  : Napoleon
        /// Created : 2015-03-25 16:25:26
        public static int ExecuteProcedureOutInt(string procedure, SqlParameter[] parameters, string intName)
        {
            int number;
            /*using (IDbConnection conn = OpenConnection())
            {
            ...
            using (IDbCommand cmd = OpenCommand(procedure, parameters))
            {
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.ExecuteNonQuery();
                number = (int)cmd.Parameters[intName];
            }
            return number;
        }
